...
child.post('/thread/reply', async (req,res)=>{
    // let user_id = req?.body?.['user_id'];
    // let password = req?.body?.['password'];
    const { user_id, tanggal, content, id_thread_parent } = req?.body;
    ...
    try {
        ...
        if (typeof row != 'undefined' && row != null){
            if (row?.['user_id'] == '' || row?.['user_id'] == null)
            {
				...
				if (!content){
						res.status(200).send({status:'success', result: null, message: 'Content tidak boleh kosong !'});
						return
				}
				...
				let p_tanggal = tanggal;
				let p_content = content;
				let p_user_id = user_id;
				let p_id_thread_parent = id_thread_parent;

				db.run('INSERT INTO Tr_Thread_Reply(tanggal, content, id_thread_parent, user_id) ' + 
								'VALUES(?, ?, ?, ?)',[p_tanggal, p_content, p_id_thread_parent, p_user_id], (err)=>{
